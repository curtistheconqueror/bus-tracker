...
test("includes full theme, manual color, highlight, and locate controls", async () => {
  ...
  assert.match(page, /CHANGE SYSTEM/);
  ...
  assert.match(css, /\.repair-choice-stage\{[^}]*height:clamp\(270px,50dvh,430px\)[^}]*max-height:none/);
  ...
  assert.match(page, /Repair catalog\. Scroll to view all categories and choices/);
  assert.match(page, /SCROLL TO VIEW ALL REPAIR OPTIONS/);
  assert.match(page, /onTouchMove=\{event=>event\.stopPropagation\(\)\}/);
  ...
  assert.match(css, /@media\(max-width:800px\)\{\.repair-choice-stage\{[^}]*\}\.defect-list\{[^}]*\}\.category-choice-grid\{grid-template-columns:repeat\(3/);
