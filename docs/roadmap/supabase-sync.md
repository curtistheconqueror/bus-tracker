...
| Client | **Built.** `src/lib/cloud/cloud-sync.ts` (pure logic), `src/lib/cloud/cloud-client.ts` (the Supabase calls), `app/cloud-sync-control.tsx` (Settings → SHOP CLOUD). Syncs the map, the Defect Log and the Down Sheet. |
