...
3. Restrict that environment to the `main` branch. Do **not** add GitHub required reviewers: Curtis approves a release in chat, and the trusted release agent records the one-file release gate. This keeps phone-first releases practical while keeping the environment's credentials unavailable to other branches. The gate is an operational trust boundary: only trusted release agents may write it.
...
2. Add a source-gated staging workflow that can only run from `main`, references the protected `production` environment, runs the existing CI gate, and deploys only after a trusted release agent records Curtis's explicit chat approval.
...
5. The release agent creates a one-file release-gate commit naming the exact approved parent source. That record starts the staging workflow.
