...
- The Cloudflare staging workflow runs only from a one-file Curtis-approved release gate on `main`. Release-capable agents may create that gate after Curtis explicitly approves in chat; they must never handle a Cloudflare token or create an automatic-on-push substitute. A separate production workflow will be added only after staging acceptance.
