...
- The Cloudflare staging workflow runs only from a one-file Curtis-approved release gate on `main`. Contributors must never create that gate, handle a Cloudflare token, or create an automatic-on-push substitute. A separate production workflow will be added only after staging acceptance.
