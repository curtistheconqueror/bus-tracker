...
- The Cloudflare staging workflow is manually dispatched and records Curtis's explicit chat approval. Contributors must never run it, handle a Cloudflare token, or create an automatic-on-push substitute. A separate production workflow will be added only after staging acceptance.
