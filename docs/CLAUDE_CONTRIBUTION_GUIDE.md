...
1. Read `README.md`, `PROJECT_HANDOFF.md`, `CONTRIBUTING.md`, this file, `docs/RELEASES.md`, `docs/PUBLISH_NEXT.md`, `docs/SITES_PUBLISHING_RUNBOOK.md`, `docs/CONTRIBUTOR_DEPLOYMENT_SETUP.md`, and `docs/PACE_PRIVATE_DEPLOYMENT_HANDOFF.md` completely.
...
- The future Cloudflare production workflow is approval-gated. Until Curtis completes the account setup described in docs/CONTRIBUTOR_DEPLOYMENT_SETUP.md, it does not exist and Claude Code must not create an unprotected substitute. After it is configured, contributors still publish only through the protected GitHub environment and never by handling a production secret directly.
