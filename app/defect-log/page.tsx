 ...
 return <div className="log-shade" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}>
  ...
  <form className="log-editor" onSubmit={submit}>
   ...
   <div className="log-form">
    ...
    <label className="wide downsheet-check deferred-check"><input type="checkbox" checked={deferred} disabled={value.defect.state==="completed"} onChange={event=>toggleDeferred(event.target.checked)}/><span><b>DEFERRED</b><small>{deferred?"Held back from service since "+timeLabel(value.defect.deferredAt||new Date().toISOString())+". Not on the Down Sheet — check DOWN SHEET above if it needs to go on.":hasHistory?"Was deferred, returned to service "+timeLabel(value.defect.deferredReturnedAt||"")+", and is still open. Check DEFERRED again to hold it back, or fix it now.":"Hold this bus back from service without putting it on the Down Sheet yet. Clears itself once this repair is marked Fixed or moved to the Down Sheet."}</small></span></label>
    {hasHistory&&<p className="deferred-history-note" role="note"><b>WAS DEFERRED</b>Returned to service {timeLabel(value.defect.deferredReturnedAt||"")} without being fixed or put on the Down Sheet. Still open.</p>}
