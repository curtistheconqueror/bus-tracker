...
function DefectEditor({draft,fleet,defaultInitials,requireInitials,partsMemory,forgetPart:forgetLearned,findingsMemory,forgetFinding:forgetLearnedFinding,save,saveFixed,close}:{draft:LogDraft;fleet:DefectLogFleetBus[];defaultInitials:string;requireInitials:boolean;partsMemory:PartsMemory;forgetPart:(entry:PartMemoryEntry)=>void;findingsMemory:FindingsMemory;forgetFinding:(entry:FindingMemoryEntry)=>void;save:(draft:LogDraft)=>void;saveFixed:(draft:LogDraft)=>void;close:()=>void}){
 ...
 const toggleOnDownSheet=(on:boolean)=>{
  setValue(current=>({...current,onDownSheet:on,defect:on&&current.defect.state==="deferred"?{...current.defect,state:"open",deferredAt:undefined,deferredUntil:undefined}:current.defect}));
 };
 ...
 return <div className="log-shade" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}>
  ...
  <form className="log-editor" onSubmit={submit}>
   ...
   <div className="log-form">
    ...
    <label className="wide downsheet-check"><input type="checkbox" checked={value.onDownSheet} disabled={value.defect.state==="completed"} onChange={event=>toggleOnDownSheet(event.target.checked)}/><span><b>DOWN SHEET</b><small>Adds it to the sheet now, without changing the bus location. Use RECOMMEND above to put it forward instead.</small></span></label>
    <label className="wide downsheet-check deferred-check"><input type="checkbox" checked={deferred} disabled={value.defect.state==="completed"} onChange={event=>toggleDeferred(event.target.checked)}/><span><b>DEFERRED</b><small>{deferred?"Held back from service since "+timeLabel(value.defect.deferredAt||new Date().toISOString())+". Not on the Down Sheet — check DOWN SHEET above if it needs to go on.":"Hold this bus back from service without putting it on the Down Sheet yet. Clears itself once this repair is marked Fixed or moved to the Down Sheet."}</small></span></label>
