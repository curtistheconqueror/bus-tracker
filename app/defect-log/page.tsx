...
import {CHECK_ENGINE_SYMPTOMS,defaultDefectOperability,defectLabel,isUnresolved,normalizeDefects,REPAIR_OPTION_GROUPS,REPAIR_OPTIONS,repairCategoryEmoji,repairCategoryLabel,type DefectOperability,type DefectState,type StructuredDefect} from "../repair-catalog";
 ...
 return <div className="log-shade" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}>
  <form className="log-editor" onSubmit={submit}>
   ...
   <div className="log-form">
    ...
    <label>QUICK SELECT (OPTIONAL)<select value={value.quickIssue} disabled={!value.defect.category} onChange={event=>{const issue=event.target.value,fanIssue=value.defect.category==="Cooling System"&&issue==="Radiator fan(s) out",oilIssue=value.defect.category==="Preventive Maintenance"&&issue==="Add engine oil";setValue(current=>({...current,quickIssue:issue,defect:{...current.defect,issue,symptoms:issue==="Check-engine diagnosis"?current.defect.symptoms||[]:[],quantity:undefined,unit:fanIssue?"fans":oilIssue?"quarts":undefined,operability:defaultDefectOperability(current.defect.category,issue)}}))}}><option value="">{value.defect.category?"Save category only or choose an issue":"Select category first"}</option>{REPAIR_OPTION_GROUPS[value.defect.category]?Object.entries(REPAIR_OPTION_GROUPS[value.defect.category]).map(([group,items])=><optgroup label={group} key={group}>{items.map(entry=><option value={group+" - "+entry} key={entry}>{entry}</option>)}</optgroup>):repairs.map(repair=><option key={repair}>{repair}</option>)}</select></label>
