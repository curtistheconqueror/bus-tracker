 ...
 return <div className="log-shade" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}>
  ...
  <form className="log-editor" onSubmit={submit}>
   ...
   <div className="log-form">
    ...
    <label className="wide downsheet-check deferred-check"><input type="checkbox" checked={deferred} disabled={value.defect.state==="completed"||value.onDownSheet} onChange={event=>toggleDeferred(event.target.checked)}/><span><b>DEFERRED</b><small>{value.onDownSheet?(deferred?"This repair is on the Down Sheet and the sheet has it deferred. Take it off the sheet to hold it back from here instead.":"Not available while this repair is on the Down Sheet. A bus cannot be on the sheet and held back off it at once."):deferred?"Held back from service since "+timeLabel(value.defect.deferredAt||new Date().toISOString())+". Not on the Down Sheet — check DOWN SHEET above if it needs to go on.":hasHistory?"Was deferred, returned to service "+timeLabel(value.defect.deferredReturnedAt||"")+", and is still open. Check DEFERRED again to hold it back, or fix it now.":"Hold this bus back from service without putting it on the Down Sheet yet. Clears itself once this repair is marked Fixed or moved to the Down Sheet."}</small></span></label>
