...
function DefectEditor({draft,fleet,defaultInitials,save,close}:{draft:LogDraft;fleet:DefectLogFleetBus[];defaultInitials:string;save:(draft:LogDraft)=>void;close:()=>void}){
 ...
 const submit=(event:React.FormEvent)=>{event.preventDefault();const initials=(value.defect.reportedBy||defaultInitials).trim().toUpperCase(),details=value.defect.details.trim(),issue=value.quickIssue||value.defect.issue;if(!selectedBus){alert("Select a bus number.");return}if(!value.defect.category){alert("Select a repair category.");return}if(!issue&&!details){alert("Select a repair or describe the symptom.");return}const finalIssue=issue&&issue!=="Manual entry"?issue:"Manual entry";if(document.activeElement instanceof HTMLElement)document.activeElement.blur();save({...value,onDownSheet:value.defect.state==="completed"?false:value.onDownSheet,defect:{...value.defect,issue:finalIssue,details,reportedBy:initials}})};
 return <div className="log-shade" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}>
  <form className="log-editor" onSubmit={submit}>
   <header><span><small>REAL-TIME DEFECT</small><h2>{selectedBus?"Bus "+selectedBus.n:"Log Repair"}</h2></span><div className="log-editor-header-actions"><button type="button" onClick={close} aria-label="Close">×</button></div></header>
   <div className="log-form">
    ...
    <div className="wide log-inline-actions" aria-label="Defect form actions"><button type="button" onClick={close}>CANCEL</button><button type="submit" className="save-log">{saveLabel}</button></div>
...
export default function DefectLog(){
 ...
 const saveDraft=(draft:LogDraft)=>{const result=saveDefectLogRecord(fleet,downEntries,draft.busId,draft.defect,draft.onDownSheet);if(result.error){alert("That bus is no longer available. Refresh and try again.");return}persist(result.fleet,result.downEntries);closeEditor()};
 ...
 return <main className="defect-log-app" style={appStyle} data-font-size={settings.fontSize}>
  ...
  {editing&&<DefectEditor draft={editing} fleet={fleet} defaultInitials={settings.defaultInitials} save={saveDraft} close={closeEditor}/>}
