...
import {defectLabel,defectWorkStates,isDiagnosticDefect,normalizeFinding,normalizeRepairHours,normalizeDefects,REPAIR_OPTIONS,repairCategoryLabel,workStateStampLabel,type DefectOperability,type StructuredDefect} from "../repair-catalog";
...
type CompletionDraft={category:string;issue:string;details:string;operability:DefectOperability;actionTaken:string;diagnosticNote:string;finding:string;repairHours:string;diagnosticHours:string;partNumber:string;partsUsed:boolean;partName:string;rememberScope?:PartMemoryScope;completedBy:string;completedAt:string};
...
function CompletionEditor({record,partsMemory,forgetPart:forgetLearned,findingsMemory,forgetFinding:forgetLearnedFinding,save,close}:{record:FixedRecord;partsMemory:PartsMemory;forgetPart:(entry:PartMemoryEntry)=>void;findingsMemory:FindingsMemory;forgetFinding:(entry:FindingMemoryEntry)=>void;save:(record:FixedRecord,draft:CompletionDraft)=>void;close:()=>void}){
 ...
 return <div className="fixed-shade" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><form className="fixed-editor" onSubmit={event=>{event.preventDefault();save(record,draft)}}>
  ...
  <div className="fixed-editor-body">
   ...
   <label>CATEGORY<select value={draft.category} onChange={event=>setDraft(current=>({...current,category:event.target.value,issue:"",partsUsed:false,partNumber:"",partName:"",rememberScope:undefined}))}>{Object.keys(REPAIR_OPTIONS).map(category=><option value={category} key={category}>{repairCategoryLabel(category)}</option>)}</select></label>
   <label>DEFECT / REPAIR<select value={draft.issue} onChange={event=>setDraft(current=>({...current,issue:event.target.value,partsUsed:false,partNumber:"",partName:"",rememberScope:undefined}))}>{draft.issue&&!issues.includes(draft.issue)&&<option>{draft.issue}</option>}<option value="">Choose repair</option>{issues.map(issue=><option key={issue}>{issue}</option>)}</select></label>
   ...
   <fieldset className={"wide billable-time"+(isDiagnosticDefect(draft.category,draft.issue)?" diagnostic":"")}><legend>BILLABLE TIME — OPTIONAL</legend>
    <div>
     ...
     <label>DIAGNOSTIC HOURS<input inputMode="decimal" value={draft.diagnosticHours} placeholder=".5" onChange={event=>update("diagnosticHours",event.target.value)}/></label>
    </div>
