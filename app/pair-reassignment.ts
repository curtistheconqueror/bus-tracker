...
export function reassignBusPair<T extends MovableRepairBus>(fleet:T[],selectedOverride:T,otherId:string,displacedTargetSlots:string[]|null,now=new Date().toISOString()):{fleet:T[];error:PairReassignmentError;displacedLocation:string|null}{
 ...
 const nextSelected={...selectedOverride,l:selectedTarget,s:statusForLocation(selectedTarget,selectedOverride.s,selectedOverride),parkedAt:now} as T;
 const nextOther={...other,l:displacedLocation,s:statusForLocation(displacedLocation,other.s,other),parkedAt:now} as T;
