...
Every printed line number that has a bus number beside it MUST produce a row. Do not skip a line because its reason is short, because the handwriting is faint, or because it looks like a repeat of the line above. Report the line number you read in lineNumber, exactly as printed, so gaps can be found. A bus number struck through or crossed out has been taken off the sheet: skip that row entirely. A row carrying several bus numbers is several rows — emit one row per bus, each with the same reason, same mechanic and its own bus number, since a line reading "PM'S 17514 17556 17525" is five-digit buses all due the same service.
