...
EVERY bus number written anywhere on the sheet MUST produce a row, and there are two kinds. Rows on the printed numbered lines: report the line number you read in lineNumber, exactly as printed, so gaps can be found. And rows written by hand OUTSIDE the table - pencilled in the margins, squeezed below the last printed line, or added in a corner - which have NO line number: emit them with lineNumber set to "margin". These handwritten rows are as real as the printed ones and are the ones most often left out; a bus written in the margin and not read reaches nobody. Do not skip any row because its reason is short, because the handwriting is faint, or because it looks like a repeat of the line above. A bus number struck through or crossed out has been taken off the sheet: skip that row entirely. A row carrying several bus numbers is several rows — emit one row per bus, each with the same reason, same mechanic and its own bus number, since a line reading "PM'S 17514 17556 17525" is five-digit buses all due the same service. REPEAT THE LINE'S WORDING ON EVERY ONE OF THOSE ROWS. The words are written once on the paper but they describe every bus on that line, so a line reading "PM'S" followed by seven bus numbers produces SEVEN rows, every one of them with reason "PM'S", category Inspection and section Inspection - never a reason left empty on the second and later buses, and never those buses falling back to the heading they sit under. The same applies to any multi-bus line: A15 with four buses is four Inspection rows reading A15, and a fault written once over two bus numbers is two rows both carrying that fault. Give every row of a multi-bus line the same lineNumber, which is the printed number that line actually carries.

The mechanic column is headed MECHANIC/LOCATION and holds either. A vendor written there — Bus & Truck, Cummins, Thermo King, Allison — is the sheet saying where the bus is, so use section Vendor Repair and put the vendor in assignedTo. A person's name there is the mechanic who has it. A row may contain several issues; keep the full wording in reason and choose the best single primary catalog category and repair below. THE REPAIR YOU CHOOSE MUST MATCH THE WORDS ON THE ROW, and it must be an entry that appears under the category you name - never a repair from one category paired with the name of another. A row reading MISFIRE CYL #5 is the Engine misfire entry, not a suspension or driveline part that happens to sit nearby in this list. When several faults are written, the FIRST one written is the primary; keep the rest in reason. When nothing in the catalog matches what is written, use Miscellaneous with Driver-reported defect and leave the wording to speak for itself - that is a better answer than a specific repair the row never mentions. A bus is OFF PROPERTY or Vendor Repair only when the sheet says so: a vendor named in the MECHANIC/LOCATION column, the words off property or offsite, or the OFF PROPERTY heading above the row. Never infer it from the kind of repair.
