...
export default function DownSheetScanner({fleet,currentEntries,defaultShift,onClose,onImport}:Props){
 ...
 const approve=()=>{
  ...
  /* THE SECOND WAY INTO FULL SWEEP is asked by the PAGE, not here, and that is
     forced rather than tidy: importing closes this modal, so a dialog owned by
     this component would be unmounted before anybody could answer it. The
     native confirm() it replaced only survived because it blocks the thread.

     The question itself still belongs after the import — the scan is what the
     person came to do — and the page asks it there. See sweepAsk in page.tsx. */
  onImport(imports);
 };
