...
export const REPAIR_OPTION_GROUPS:Record<string,Record<string,string[]>>={
 ...
 "Bus Accessories":{
  ...
  "Wipers":["Wiper blade (curbside)","Wiper blade (roadside)","Wiper motor (curbside)","Wiper motor (roadside)","Other wiper defect"],
 },
 ...
 "Tech Services":{
  "Farebox":["INOP (general)","No power","Blank / black screen","Bill transport INOP","Coin mech INOP","Coin off line","Coin bin missing","Unlocked / won't lock","Can't unlock top / coin bypass reset","Won't probe & open","Loose from floor mounts","Other farebox defect"],
 ...
 "Amerex":{
  "Fire Suppression":["FIRE alarm (system discharged)","Heat sensor communication fault","Trouble Mod 1 Roof 1","Trouble Mod 1 Roof 2","Trouble Mod 1 Engine","Trouble Mod 2 Roof 1","Trouble Mod 2 Roof 2","Trouble Mod 2 Engine","Control head no power","Other Fire Suppression Trouble"],
...
/* The Amerex Vehicle SafetyNet control head is two systems behind one faceplate,
   ...
   Fire Suppression is heat sensors set in sections at the rear, where the CNG
   lines run, reported by module: Mod 1 and Mod 2, each covering Roof 1, Roof 2
   and the engine. The engine positions were missing from the picker, so a
   mechanic reading TROUBLE MOD 1 ENGINE off the panel had nothing to file it
   as. It fires on its own with no operator input at all, so FIRE on this panel
   means the bottles have already gone off, not that somebody needs to decide
   something. Trouble is the amber one: most often a sensor that has stopped
   answering.
